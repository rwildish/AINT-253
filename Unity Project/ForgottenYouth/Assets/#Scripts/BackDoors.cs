...
public class BackDoors : MonoBehaviour {
    ...
    public void CheckDoor()
    {
        if (Physics.Raycast(cam.position, cam.forward, out hit, 10, mask))
        {
            ...
            open = false;
            if (hit.transform.localRotation.eulerAngles.y > 135)
            {
                open = true;
    ...
    public void OpenAndCloseDoor()
    {
        ...
        {
            currentDoor.localRotation = Quaternion.Slerp(currentDoor.localRotation, Quaternion.Euler(0, 180, 0), timeLeft / 4);
        }
